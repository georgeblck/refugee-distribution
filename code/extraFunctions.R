get.ref.numbers <- function(input.list, year.range = c(2014, 2015), which.source = "unhcr", 
                            countries = c("1", "2")){
  # Extract data
  ...
  if (!all.eu) total.year <- total.year[which(!total.year$country%in%non.eu), ]
  if (!all.dublin) total.year <- total.year[which(!total.year$country%in%non.dublin), ]
  
  # Get the number of Asyl Applicants in the time range (for the specified source!)
  applic.year   <- subset(asyl.ls[[which.source]], 
                          (country %in% total.year$country) & (year %in% (year.range[1]:year.range[2])))
  all.applic    <- sum(aggregate(applic ~ country, applic.year, sum)$applic, na.rm = TRUE)
  
  # Get the number of Accepted Asyl Applicants (only one source)
  accept.year <- subset(accept, (country %in% total.year$country) & (year %in% (year.range[1]:year.range[2])))
  all.accept  <- sum(aggregate(accept ~ country, accept.year, sum)$accept, na.rm = TRUE)
...
get.ref.table <- function(input.list, year.range = c(2014, 2015), which.source = "unhcr", 
  ...
  quota.applic  <- round(num.applic$applic/sum(num.applic$applic, na.rm = FALSE), 4)
  ...
  res.key <- calc.key(total.year, which = which.idx, weight.pop = w.pop, 
                      ...
                      weight.asyl = w.asyl, number = all.applic)
  res.mat <- merge(res.key, res.applic, by = "country")
  
  # Get the number of accepted Asyl Applicants in the time range (only Eurostat)
  accept.year <- subset(accept, (country %in% total.year$country) & (year %in% (year.range[1]:year.range[2])))
  num.accept  <- aggregate(accept ~ country, accept.year, sum)
  quota.accept <- round(num.accept$accept/sum(num.accept$accept, na.rm = FALSE), 4)
...
# Function to calculate the different refugee distribution keys that will be plotted
calc.key <- function(x, weight.pop = 0.4, weight.gdp = 0.4, which = "grech", cap = 0.3,
                     weight.unemp = 0.1, weight.asyl = 0.1, number = 1000000){
  # Number of refugees to distribute
  if (which == "grech"){
    unemp.effect  <- 1 + weight.unemp * (1 - (x$unemp / mean(x$unemp, na.rm = TRUE)))
    asyl.effect   <- 1 + weight.asyl * (1 - (x$asyl / mean(x$asyl, na.rm = TRUE)))
    base.effect   <- 0.5 * ((x$pop / sum(x$pop, na.rm = TRUE)) + (x$gdp / sum(x$gdp, na.rm = TRUE)))
    quota.key <- (unemp.effect * asyl.effect * base.effect) / 
      sum((unemp.effect * asyl.effect * base.effect), na.rm = TRUE) 
    key <- quota.key * number
  } else if (which == "konig"){
    quota.key   <- as.matrix(weight.pop * (x$pop / sum(x$pop, na.rm = TRUE)) + 
                           weight.gdp * (x$gdp / sum(x$gdp, na.rm = TRUE)))
    key <- quota.key * number    
  } else if (which == "eu"){
    ...
    # Calculate the effect of the past number of Asyl Applications
    ...
    alloc.cap <- number * (weight.pop * pop.effect + weight.gdp * gdp.effect + 
                             weight.asyl*asyl.effect + weight.unemp * unemp.effect)
    alloc.res <- (number - sum(alloc.cap, na.rm = TRUE)) * (0.5 * base.effect)
...
# Rolling average from left to calculate the average asylum applications 
# per 1mil Population of the preceeding X years
...
calc.asyleffect <- function(asyl, population, name.app = "applic", 
                            name.pop = "POP", name.geo = "GEO",
                            name.time = "TIME"){
  merged  <- merge(asyl, population, by = c("GEO", "TIME"))
  pure.asyl   <- merged[,c(name.app)]/round(merged[,c(name.pop)]/1000000, 2)
  asyleffect  <- unlist(by(pure.asyl, merged[, c(name.geo)], rollAvg))
...
read.eurostat <- function(file, name){
  temp <- read.table(file, header = TRUE, sep =",", quote="\"", dec = ".", na.strings = ":")
  temp$Value <- as.numeric(gsub(",","",temp$Value))
  colnames(temp)[which(colnames(temp)=="Value")] <- name
  if ("GEO" %in% colnames(temp)) temp$GEO <- gsub("^Germany \\(until.*$", "Germany", temp$GEO)
  return(temp)
}
